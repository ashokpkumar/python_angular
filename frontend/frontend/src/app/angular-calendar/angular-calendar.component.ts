...
import {AngularCalendarService} from './angular-calendar.services'
const colors: any = {
  red: {
    primary: '#ad2121',
    secondary: '#FAE3E3',
...
export class AngularCalendarComponent implements OnInit {
  ...
  constructor(private modal: NgbModal,private apiService: AngularCalendarService) {}
  ...
  refreshPage({ date, events }: { date: Date; events: CalendarEvent[] }): void {
    console.log("Coming inside")
    if (isSameMonth(date, this.viewDate)) {
      if (
        (isSameDay(this.viewDate, date) && this.activeDayIsOpen === true) ||
  ...
  getEvents(){
    this.apiService.getEvents().subscribe(data=>{console.log("Time Data: ",data),
                                                this.events = data,
                                                console.log(this.events[1])
